...
async def level(ctx, *, posname = None):
    if posname is not None:
        try:
            ...
            lvlname = [lvl["name"] for lvl in deml.find() if lvl["name"].lower() == posname.lower()]
            if len(lvlname) > 0:
...
@client.command(aliases=["постирон", "postironiya", "постирония"])
async def postiron(ctx):
    ...
    await ctx.send(file=discord.File('postironiya.jpg'))
...
async def help(ctx, arg=None):
    if arg in ["рулетка", "roulette", "r", "р"]:
        ...
        await browse_pages(ctx, arg, 2, [embed1,embed2], False)
    else:
        await ctx.send("чо творишь")
